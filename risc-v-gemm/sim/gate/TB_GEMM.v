...
`define TCK_NS                4         // 200MHz (5ns)
`define MAX_LATENCY_NS        8000000   // 10 ms (소프트웨어 mul32에 충분히 큼)
`define RESULT_CHECK_DELAY_NS 0          // DONE 이후 결과 읽기 전 추가 대기 시간

`define MAX_CYCLES 2000000  // 2,000,000 cycles
...
module TB_GEMM;
  ...
  //--------------------------------------------------------------------------
  initial begin
    $sdf_annotate("./../../syn/outputs/RISCVSINGLE_gate.sdf", DUT.CPU);
    pass_count = 0;
    fail_count = 0;
