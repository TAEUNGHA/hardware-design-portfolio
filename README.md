...
- 검증을 **SW → PS → PS+PL → PL** 단계로 나눠 PL이 PS 결과를 **bit-exact 재현**함을 확인
...
- **5-stage 파이프라인 RISC-V**에 MUL 명령어와 8×8 행렬곱 연산 구현
- 데이터·제어 해저드를 **명령어 재정렬 / predict-not-taken + flush**로 해결
...
- CNN convolution을 병렬 가속하는 **3×3 멀티코어 프로세서**를 SystemVerilog로 설계
- 각 코어에 stack 구조를 두어 **systolic 데이터플로우**로 weight·input 재사용 극대화
